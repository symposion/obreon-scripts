...
module.exports = class JournalWrapper {
  ...
  static getWrapperPromise(roll20, reporter, handout, logger) {
    return new Promise(resolve => {
      handout.get('gmnotes', gmnotes => {
        const journal = Journal.parseFromJSON(gmnotes);
        logger.wrapModule(journal);
        resolve(new JournalWrapper(journal, roll20, reporter, handout));
      });
    });
  }
