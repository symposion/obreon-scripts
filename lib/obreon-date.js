...
module.exports = class ObreonDate {
  ...
  get bareDate() {
    return this.without(['minute', 'hour']);
  }

  get dayName() {
    return ObreonDate.DaysOfWeek[(this.day % 6)];
  }

  get monthName() {
    return ObreonDate.Months[this.month - 1];
  }
  ...
  toLongString() {
    ...
    if (!_.isUndefined(this.day)) {
      ...
      string += `${this.dayName} ${ObreonDate.getOrdinal(this.day)}`;
    }
    if (!_.isUndefined(this.month)) {
      string = string && `${string} of `;
      string += `${this.monthName}`;
    }
  ...
  static fromString(dateString, defaults) {
    ...
    const match = dateString.trim().match(re);
