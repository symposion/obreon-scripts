...
class Journal {
  ...
  activity(duration, description, xp) {
    this.addEntry(new ActivityEntry(this.end, this.end.advance(duration), description, xp));
  }

  addPartyMembers(characterIds) {
    const existing = this.getPartyMemberIds();
    const newChars = _.difference(characterIds, existing);
    if (newChars.length) {
      const newMembers = existing.concat.apply(existing, newChars);
      this.addEntry(new PartyChangeEntry(this.end, newMembers, newChars, 'joined'));
    }
  }
  ...
  getPartyMemberIds() {
    const partyChangeIndex = _.findLastIndex(this._entries, entry => entry instanceof PartyChangeEntry);
    if (partyChangeIndex !== -1) {
      return this._entries[partyChangeIndex]._partyMemberIds;
    }
    return [];
  }
  ...
  static get entryTypes() {
    return [TravelEntry, WeatherEntry, ActivityEntry, PartyChangeEntry].reduce((lookup, constructor) => {
  ...
  toJSON() {
    return _.omit(this, 'isLogWrapped');
  }
...
class JournalEntry {
  ...
  toJSON() {
    return _.defaults({ type: this.constructor.name }, _.omit(this, 'isLogWrapped'));
  }

}

class PartyChangeEntry extends JournalEntry {
  constructor(time, partyMemberIds, changed, changeType) {
    super(time);
    this._partyMemberIds = partyMemberIds;
    this._changed = changed;
    this._changeType = changeType;
  }

  render(message) {
    const text = `<em style="font-size:larger">${this.getTimeSpecFor(false)}: </em>` +
      `${this._changed.map(message.lookupCharacterName.bind(message)).join(', ')} ${this._changeType} ` +
      'the party. The party now consists of: ' +
      `${this._partyMemberIds.map(message.lookupCharacterName.bind(message)).join(', ')}`;
    message.paragraph(text);
  }

  static constructorParams(object) {
    return [object._start, object._partyMemberIds, object._changed, object._changeType];
  }

  static get logWrap() {
    return 'PartyChangeEntry';
  }
}
...
class WeatherEntry extends JournalEntry {
  ...
  static constructorParams(object) {
    return [object._start, object._weather];
  }
...
class ActivityEntry extends JournalEntry {
  constructor(start, end, text, xp, stage) {
  ...
  static constructorParams(object) {
    return [object._start, object._end, object._text, object._xp, object._stage];
  }
