...
module.exports = class ObreonScripts {
    ...
    const wrapper = {
      ...
      getFinalStatusPromise() {
        return Promise.all([this.getWeatherPromise(), this.getEntries()])
          .then(results => {
            const lastTimedEntry = results[1].reverse().find(_.property('start'));
            const lastLocationEntry = results[1].reverse().find(_.property('location'));
            return {
      ...
      getSummaryTextPromise() {
        return this.getFinalStatusPromise()
          .then(statusInfo =>
            ...
            `You are at ${statusInfo.lastLocation}. ${statusInfo.weather.getWeatherText()}`
          );
      },
  ...
  recordActivity(options) {
    ...
    function addLocation(journalEntry) {
      return options.location ? currentJournal.changeLocation(options.location) : journalEntry;
    }
    ...
    return currentJournal.getFinalStatusPromise()
      .then(status => {
        ...
        const makeNewJournalPromise = this.getNewJournalPromise(end, currentJournal)
          .then(nextJournal => {
            if (shouldAddEntryToNewJournal) {
  ...
  getNewJournalPromise(end, previousJournal) {
